...
from .agent.agent_loop_state import AgentLoopState, AgentLoopStateManager

logger = logging.getLogger(__name__)


class AgentLoop:
    """Manages the execution loop for an agent."""

    def __init__(self):
        self.state_manager = AgentLoopStateManager()
    ...
    async def run(
        ...
        try:
            ...
            while True:
                if self.stop_run_event.is_set() or self.state_manager.should_stop():
                    logger.info("Stop signal received. Exiting execute_run loop.")
                    break

                # Process queued messages
                await self._process_queued_messages(agent, callback)
                ...
                self.state_manager.metrics.record_message()
                ...
                self.state_manager.metrics.record_tool_call()
    ...
    async def stop(self):
        ...
        if self.execute_run_task and not self.execute_run_task.done():
            ...
            self.state_manager.state = AgentLoopState.STOPPING
            self.stop_run_event.set()  # Set stop event before changing state
            ...
            try:
                ...
                # Ignore other exceptions during stop
                pass

            finally:
                self.execute_run_task = None
                self.state_manager.state = AgentLoopState.STOPPED
                logger.info("Run has been stopped.")
        else:
            logger.info("No active run to stop.")

    async def _process_queued_messages(self, agent: Agent, callback: Optional[Callable]):
    ...
    def get_metrics(self) -> dict:
        """Get current metrics from the state manager."""
